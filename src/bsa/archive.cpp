...
bool Archive::write_tes3(Path path) && noexcept
{
    ...
    for (auto &[path, file] : std::move(files_))
    {
        auto tes3_file = std::move(file).as_raw_file<libbsa::tes3::file>();
        assert(tes3_file && "Invalid file in tes3 archive");
        bsa.insert(path, std::move(*tes3_file));
    }
    return do_write(BTU_MOV(bsa), [](auto &&bsa, auto &&path) { bsa.write(BTU_FWD(path)); }, BTU_MOV(path));
}
...
bool Archive::write_fo4(Path path) && noexcept
{
    ...
    for (auto &[path, file] : std::move(files_))
    {
        ...
        ba2.insert(path, std::move(*fo4_file));
    }
    return do_write(
        BTU_MOV(ba2),
...
auto Archive::write(const Path &path) && -> bool
{
    if (files_.empty())
        return false;
